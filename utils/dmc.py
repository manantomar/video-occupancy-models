...
class ExtendedTimeStepEval(NamedTuple):
    step_type: Any
    reward: Any
    discount: Any
    observation: Any
...
class FrameStackWrapper(dm_env.Environment):
    ...
    def _extract_pixels(self, time_step):
        try:
            pos = time_step.observation['position']
        except:
            pos = None
        ...
        if len(pixels.shape) == 4:
            pixels = pixels[0]
        return pixels.transpose(2, 0, 1).copy(), pos

    def reset(self):
        time_step = self._env.reset()
        pixels, pos = self._extract_pixels(time_step)
        for _ in range(self._num_frames):
            self._frames.append(pixels)
        return self._transform_observation(time_step), pos
...
class ExtendedTimeStepWrapper(dm_env.Environment):
    ...
    def reset(self):
        time_step, pos = self._env.reset()
        return self._augment_time_step(time_step), pos

    def step(self, action):
        time_step, pos = self._env.step(action)
        return self._augment_time_step(time_step, action), pos

    def _augment_time_step(self, time_step, action=None):
        if action is None:
            action_spec = self.action_spec()
            action = np.zeros(action_spec.shape, dtype=action_spec.dtype)
        return ExtendedTimeStepEval(observation=time_step.observation,
...
def make(name, frame_stack, action_repeat, seed, distracting_mode: str = None, multitask_mode: str = None):
    ...
    if (domain, task) in suite.ALL_TASKS:
        ...
        if distracting_mode is not None:
            ...
            kwargs['pixels_only'] = False
            kwargs['render_kwargs'] = render_kwargs
            kwargs['background_dataset_path'] = "/home/manant/scratch/DAVIS/JPEGImages/480p/" #"DAVIS/JPEGImages/480p/"
