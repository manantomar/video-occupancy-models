...
from vqkd_teacher import get_dino_vit_base#, clip
...
class VQKD(nn.Module):
    ...
    def pre_process(self, data):
        print("pre process called", data.shape)
    ...
    def get_codebook_entry(self, ind):
        ind_shape = ind.shape
        ind = ind.reshape(-1)
        z = self.quantize.embedding(ind)
        z = z.reshape(ind_shape[0], -1)
        return z
    ...
    def encode(self, x):
        ...
        # print("before quant and after shapes", to_quantizer_features.shape, quantize.shape)
